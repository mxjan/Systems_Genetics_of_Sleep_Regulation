...
BEGIN
  writeln(OUTf,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12,'-1.000':12);
END;
...
BEGIN
  ...
  tot:=0; FOR hz:=3 TO 320 DO tot:=tot+spec[3,hz];

  {delta   1.00 - 4.25 Hz}
  band:=0; FOR hz:= 4 TO 17   DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {theta   5 - 9 Hz}
  band:=0; FOR hz:=20 TO 36   DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {sigma   11.00 - 16.00 Hz}
  band:=0; FOR hz:=44 TO 64   DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {beta    18.00 - 30 Hz}
  band:=0; FOR hz:=72 TO 120  DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {gamma-1 32.00- 55 Hz}
  band:=0; FOR hz:=128 TO 220 DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {gamma-2 55.25 - 80.00 Hz}
  band:=0; FOR hz:=221 TO 320 DO band:=band+spec[3,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
...
BEGIN
  ...
  IF (nID=92) AND (m=1) then write_empty else bands;
  /*
  CASE nID OF
...
BEGIN
  ...
  assign(OUTf,'D:\BXD Sinergia\phenotypes\BXD_REM EEG bands.out'); rewrite(OUTf);
  writeln(OUTf,'nID':4,'line':12,'mouseID':11,'m#':5,'room':5,'rec':5,'worm':5,
               'delta':12,'delta%':12,'theta':12,'theta%':12,'sigma':12,'sigma%':12,'beta':12,'beta%':12,'gamma1':12,'gamma1%':12,'gamma2':12,'gamma2%':12);
