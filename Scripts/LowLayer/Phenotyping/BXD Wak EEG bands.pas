...
BEGIN
  ...
  tot:=0; FOR hz:=3 TO 320 DO tot:=tot+spec[1,hz];

  {delta   1.00 - 4.25 Hz}
  band:=0; FOR hz:= 4 TO 17   DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {theta   6.00 - 10.50 Hz}
  band:=0; FOR hz:=24 TO 42   DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {sigma   11.00 - 16.00 Hz}
  band:=0; FOR hz:=44 TO 64   DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {beta    18.00 - 30 Hz}
  band:=0; FOR hz:=72 TO 120  DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {gamma-1 32.00- 55 Hz}
  band:=0; FOR hz:=128 TO 220 DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
  {gamma-2 55.25 - 80.00 Hz}
  band:=0; FOR hz:=221 TO 320 DO band:=band+spec[1,hz]; write(OUTf,band:12:4,band*100/tot:12:3);
...
BEGIN
  ...
  assign(OUTf,'D:\BXD Sinergia\phenotypes\BXD_Wak EEG bands.out'); rewrite(OUTf);
