...
BEGIN
  ...
  assign(OUTf,'D:\BXD Sinergia\phenotypes\BXDsleepepisodesSD.out'); rewrite(OUTf);
  writeln(OUTf,'nID':4,'line':12,'mouseID':11,'m#':5,'room':5,'rec':5,'worm':5,'num_sleep':12,'dur_sleep':12);
  ...
  BEGIN
    ...
    BEGIN
      ...
      REPEAT
        ...
        IF spec4.state in ['n','2','5','r','3','6'] THEN
        BEGIN
          inc(nums);
          local:=1;
          REPEAT
            inc(t); read(EEGf,spec4);
            IF spec4.state in ['n','2','5','r','3','6'] THEN inc(local) ELSE ends:=true;
          UNTIL ends;
          durs:=durs+local;
        END;
      UNTIL t>=5400;
