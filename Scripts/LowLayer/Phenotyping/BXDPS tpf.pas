...
BEGIN
  ...
  IF (peakf>22) AND (peakf<44) THEN                                          {peak frequency has to be >6.5 Hz and <12 Hz}
  BEGIN
...
BEGIN
  ...
  assign(OUTf,'D:\BXD Sinergia\phenotypes\BXD_PS tpf.out'); rewrite(OUTf);
   writeln(OUTf,'nID':4,'line':12,'mouseID':11,'m#':5,'room':5,'rec':5,'worm':5,'TPF_Lb':12,'TPF_Lr1':12,'TPF_Lr2':12,'TPF_r1d':12,'TPF_r2d':12);
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        tpf[day]:=0.0; tdw:=0;
        seek(EEGf,start+(day-1)*21600);  {tpf in each L-period}
        IF day=3 THEN
        BEGIN
