...
print "
module add UHTS/Aligner/STAR/2.4.0.g;
";
...
foreach my $SAMPLE (sort keys(%Fastq)) {
  ...
  foreach my $lane (sort keys(%{$Fastq{$SAMPLE}})) {
    ...
    my $command1 = "bsub -q dee-hugemem -o $output/1pass_$SAMPLE.out -e $output/1pass_$SAMPLE.err -n 16 -R \"span[hosts=1]\" -R \"rusage[mem=$memory]\" -M $memoryKb \"STAR --genomeDir $BTgenome --outFilterIntronMotifs RemoveNoncanonicalUnannotated --readFilesCommand zcat --outFileNamePrefix 1pass_$SAMPLE --runThreadN 16 --readFilesIn ";
    my $command2 = join(",", @R1);
    #my $command3 = join(",", @R2);
    my $command4 = "--outSAMtype None\"\n";
