...
Rscript RmdTomd.R HivePlot_v3.0
...
```{r, echo=TRUE}
##setwd("//nas.unil.ch/CIG/GROUPS/FRANKEN/common/PROJECTS/BXD/Analysis/Reproducibility/HivePlots")
DataDir <- "Data/" # location of data files (QTLs, transformed raw data, ...)
TMPDir <- "TMP/" # location of precomputed files
...
formatData <- function(Expr,cis){
  ...
  GenotypeMatrix <- read.table(paste(DataDir,"Genotype.FormatedName.geno",sep=''), header=TRUE)
  Genepos <- read.table(paste(DataDir,"RefSeqMiddlePosition.table",sep=''), header=TRUE)
...
load(paste(DataDir,"SleepPhenotype3.Rdata",sep=''))

# Load data expression
CNSD <- read.table(paste(DataDir,"htseq-count_summary.txt.NSD.CPMnormalizedWITHNSDSD.txt",sep=''),header=TRUE,row.names=1)
CNSD <- CNSD[,rownames(traitData)] # to sort columns (BXD lines) by their order (rows) for the traitData
CSD <- read.table(paste(DataDir,"htseq-count_summary.txt.SD.CPMnormalizedWITHNSDSD.txt",sep=''),header=TRUE,row.names=1)
CSD <- CSD[,rownames(traitData)]
LNSD <- read.table(paste(DataDir,"htseq-count_summary.Liver.txt.NSD.CPMnormalizedWITHNSDSD.txt",sep=''),header=TRUE,row.names=1)
LNSD <- LNSD[,rownames(traitData)]
LSD <- read.table(paste(DataDir,"htseq-count_summary.Liver.txt.SD.CPMnormalizedWITHNSDSD.txt",sep=''),header=TRUE,row.names=1)
LSD <- LSD[,rownames(traitData)]

# Load Phenotype categories, change some names
# sleep phenotypes
Pheno <- read.table(paste(DataDir,"Phenotype.Categories.txt",sep=''),sep='\t',header=TRUE)
...
File <- paste(DataDir,"Metabolites_BXD_alldata.txt",sep='')
...
mNSD <- read.table(paste(DataDir,"MetabolitesMean.NSD.txt",sep=''),header=TRUE,row.names=1)
mNSD <- mNSD[rownames(traitData),]
# Get SD metabolite mean concentration:
mSD <- read.table(paste(DataDir,"MetabolitesMean.SD.txt",sep=''),header=TRUE,row.names=1)
colnames(mSD) <- sapply(colnames(mSD),paste,'_SD',sep='')
mSD <- mSD[rownames(traitData),]
# Get FC metabolite mean concentration:
mFC <- read.table(paste(DataDir,"MetabolitesFC.txt",sep=''),header=TRUE,row.names=1)
colnames(mFC) <- sapply(colnames(mFC),paste,'_FC',sep='')
mFC <- mFC[rownames(traitData),]

# Corticosteroid data
Cort <- read.table(paste(DataDir,"Corticosteroid_Unfiltered.txt",sep=''),header=TRUE,row.names=1)
...
cisCNSD <- read.table(paste(DataDir,"ciseQTL.Cortex.NSD.txt",sep=''))
qcisCNSD <- eQTLCorrection(cisCNSD)

cisCSD <- read.table(paste(DataDir,"ciseQTL.Cortex.SD.txt",sep=''))
qcisCSD <- eQTLCorrection(cisCSD)

cisLNSD <- read.table(paste(DataDir,"ciseQTL.Liver.NSD.txt",sep=''))
qcisLNSD <- eQTLCorrection(cisLNSD)

cisLSD <- read.table(paste(DataDir,"ciseQTL.Liver.SD.txt",sep=''))
qcisLSD <- eQTLCorrection(cisLSD)

# Read transeQTL
transCNSD <- read.table(paste(DataDir,"transeQTL_filtered_C_NSD.txt",sep=''),header=TRUE)
transCNSD <- transCNSD[transCNSD$p_value<=transthreshold,]

transCSD <- read.table(paste(DataDir,"transeQTL_filtered_C_SD.txt",sep=''),header=TRUE)
transCSD <- transCSD[transCSD$p_value<=transthreshold,]

transLNSD <- read.table(paste(DataDir,"transeQTL_filtered_L_NSD.txt",sep=''),header=TRUE)
transLNSD <- transLNSD[transLNSD$p_value<=transthreshold,]

transLSD <- read.table(paste(DataDir,"transeQTL_filtered_L_SD.txt",sep=''),header=TRUE)
transLSD <- transLSD[transLSD$p_value<=transthreshold,]

# Marker Position File
Mpos <- read.table(paste(DataDir,"Genotype.FormatedName.geno",sep=''),header=TRUE)
Locus <- Mpos$Locus
Mpos <- 1:nrow(Mpos)
names(Mpos) <- Locus

# Limma data
LimmaC <- as.matrix(read.table(paste(DataDir,"Limma_Cortex.txt",sep='')))
LimmaL <- as.matrix(read.table(paste(DataDir,"Limma_Liver.txt",sep='')))
...
# All genes
Allgenes <- read.table(paste(DataDir,"RefSeqMiddlePosition.table",sep=''),header=TRUE,row.names=1)
...
if (file.exists(paste(DataDir,PRECOMPUTEDFILENAME,sep=''))){
  load(paste(DataDir,PRECOMPUTEDFILENAME,sep=''))
}else{
  # Create correlation matrix and filter using setup hard thresholds (variable finished by 1 _> SD; 2 -> NSD)
  # Cortex~Liver
  #Tcor1 <- cor(t(CSD),t(LSD),method="pearson")
  load("Data/pcor_spearman_CortexSD_LiverSD.Rdata")
  Tcor1 <- t
...
SetRadiusGenes <- function(Tissue){
  ...
  rUp <- ((rUp-min(rUp))/(max(rUp)-min(rUp)))*(1000-(1000-newmin))+(1000-newmin)
  rNotSignif <- rank(NotSignif,ties="random")
  rNotSignif <- ((rNotSignif-min(rNotSignif))/(max(rNotSignif)-min(rNotSignif)))*((min(rUp)-60)-(max(rDown)+60))+(max(rDown)+60)
  radius <- c(rDown,rUp,rNotSignif)
