plotHive2 <- function(HPD,main=NULL,p1=1,p2=2, ch = 1, method = "abs",
...
##### Three dimensional case (using grid graphics)
	...
	if (nx == 3) {
		...
		#r.st<-1
		axst <- c(angle1, angle2, angle3)
		x0a = p2cX(r.st, axst)
		y0a = p2cY(r.st, axst)

		r.end <- c(max1, max2, max3)
		#r.end<-2000
		axend <- c(angle1, angle2, angle3)
		x1a = p2cX(r.end, axend)
		y1a = p2cY(r.end, axend)
	...
	# Set up grid graphics viewport
		...
		if (np) grid.newpage()
		md<-2000
		vp <- viewport(height=.5,width=.5,layout.pos.row = p1,layout.pos.col = p2,x = .5, y = .5,
			xscale = c(-md+900, md-900), yscale = c(-md+800, md+100), name = "3DHivePlot") # 800 800 600 200
		grid.rect(gp = gpar(col = NA, fill = "#777776"),vp=vp,default.units = "native")
		grid.circle(r=1050,gp = gpar(col = NA, fill = "black"),vp=vp,default.units = "native")
		#print("a")
	...
	# Axis 1 -> 2
		...
		for (n in 1:nrow(edges)) {
			...
			if ((nodes$axis[id1[n]] == 1) & (nodes$axis[id2[n]] == 2)) {
				th.st <- c(th.st, angle1)
				r.st <- c(r.st, nodes$radius[id1[n]])
				th.end <- c(th.end, angle2)
				r.end <- c(r.end, nodes$radius[id2[n]])
				ecol <- c(ecol, edges$color[n])
				ewt <- c(ewt, edges$weight[n])
				}
		...
		if (!length(x0) == 0) {
			grid.curve(x0, y0, x1, y1,
				default.units = "native", ncp = 5, square = FALSE,
				gp = gpar(col = ecol, lwd = ewt), curvature = 0.5,vp=vp)
			}
	# Axis 1 -> 3
	...
	# Axis 3 -> 1
		...
		for (n in 1:nrow(edges)) {
			...
			if ((nodes$axis[id1[n]] == 3) & (nodes$axis[id2[n]] == 1)) {
				th.st <- c(th.st, angle3)
				r.st <- c(r.st, nodes$radius[id1[n]])
				th.end <- c(th.end, angle1)
				r.end <- c(r.end, nodes$radius[id2[n]])
				ecol <- c(ecol, edges$color[n])
				ewt <- c(ewt, edges$weight[n])
				}
	...
	# Now add nodes
		...
		if (dr.nodes) {
			...
			theta <- c(rep(angle1, length(n1$radius)),
				rep(angle2, length(n2$radius)),
				rep(angle3, length(n3$radius)))
			x = p2cX(r, theta)
			y = p2cY(r, theta)
			grid.points(x, y, pch = 21, gp = gpar(cex = c(n1$size, n2$size, n3$size),
			black = "black",fill = c(n1$color, n2$color, n3$color)),vp=vp)
			}
	...
	# Now label axes
		...
		if (!is.null(axLabs)) {
			...
			if (is.null(axLab.pos)) axLab.pos <- r*0.1
			r <- r + axLab.pos
			th <- c(angle1, angle2, angle3)
			x <- p2cX(r, th)
			y <- p2cY(r, th)
			grid.text(axLabs,vp=vp, x, y, gp = axLab.gpar,  default.units = "native", ...)
			}
	...
	# Add a legend arrow & any annotations
		...
		if (!is.null(anNodes)) annotateNodes(anNodes, nodes, nx, anCoord)
		if (!is.null(grInfo)) addGraphic(grInfo, nodes, nx, anCoord)
		grid.text(main, x=0.5, y=0.05,
          gp=gpar(fontsize=5, col="grey"),vp=vp)
		} # end of 3D
...
##### Five dimensional case (using grid graphics)
	...
	if (nx == 5) {
		...
		axst <- c(angle1, angle2, angle3, angle4, angle5)
		x0a = p2cX(r.st, axst)
		y0a = p2cY(r.st, axst)

		r.end <- c(max1, max2, max3, max4, max5)
		axend <- c(angle1, angle2, angle3, angle4, angle5)
		x1a = p2cX(r.end, axend)
		y1a = p2cY(r.end, axend)
	...
	# Set up grid graphics viewport
		...
		#vp <- viewport(x = 0.5, y = 0.5, width = 1, height = 1,
		#	xscale = c(-md, md), yscale = c(-md, md), name = "3DHivePlot")
		grid.rect(gp = gpar(col = NA, fill = "grey20"),default.units = "native")
		grid.circle(r=1050,gp = gpar(col = NA, fill = "black"),default.units = "native")
	# Now draw edges (must do in sets as curvature is not vectorized)
	...
	# Axis 4 -> 5
		...
		for (n in 1:nrow(edges)) {
			...
			if ((nodes$axis[id1[n]] == 4) & (nodes$axis[id2[n]] == 5)) {
				th.st <- c(th.st, angle4)
				r.st <- c(r.st, nodes$radius[id1[n]])
				th.end <- c(th.end, angle5)
				r.end <- c(r.end, nodes$radius[id2[n]])
				ecol <- c(ecol, edges$color[n])
				ewt <- c(ewt, edges$weight[n])
				}
	...
	# Now add nodes
		...
		if (dr.nodes) {
			...
			theta <- c(rep(angle1, length(n1$radius)),
				rep(angle2, length(n2$radius)),
				rep(angle3, length(n3$radius)),
				rep(angle4, length(n4$radius)),
				rep(angle5, length(n5$radius)))
			x = p2cX(r, theta)
			y = p2cY(r, theta)
			grid.points(x, y, pch = 20, gp = gpar(cex = c(n1$size, n2$size, n3$size, n4$size, n5$size),
			col = c(n1$color, n2$color, n3$color, n4$color, n5$color)),vp=vp)
			}
	...
	# Now label axes
		...
		if (!is.null(axLabs)) {
			...
			if (is.null(axLab.pos)) axLab.pos <- r*0.1
			r <- r + axLab.pos
			th <- c(angle1, angle2, angle3, angle4, angle5)
			x <- p2cX(r, th)
			y <- p2cY(r, th)
			grid.text(axLabs,vp=vp, x, y, gp = axLab.gpar,  default.units = "native", ...)
			}
