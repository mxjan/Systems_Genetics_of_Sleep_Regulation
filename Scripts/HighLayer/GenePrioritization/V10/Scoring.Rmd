...
>
>Related paper: The molecular consequences of sleep loss: a systems genetics approach, PMID: 1234
...
if (is.null(opt$Log)){LogTransform<<-F} else {LogTransform<<-T}
...
```{r}
#save(PhenoID,Condition,Tissu,PhenotypesType,ScorePermutation,QTLPerm,CORES,WeightingMethod,Discretization,LogTransform,opt,file="option.Rdata")
```
...
if (LogTransform == T){
  AllPheno[,PhenotypeName]<-log2(AllPheno[,PhenotypeName])
}
...
```{r}
#Mice<-colnames(read.table("C:/Users/Maxime Jan/switchdrive/Project/GeneScoring_mm10/data/edgeR_normalized_log2CPM_Cortex_NSD.tab",header=T))
...
Mice<-Mice[-grep("BXD005|BXD029|BXD032|BXD063|B6|DB|BD",Mice)] # get mices, remove old strains, F1 and BXD063
...
OutputScore<-paste(gsub(".Rmd",'',opt$input),opt$id,opt$condition,opt$tissu,opt$phenotypetype,opt$weighting,opt$discretization,"QTL.pdf",sep=".")
pdf(file=OutputScore,width = 10,height = 5)
QTLPlot()
dev.off()
```

```{r}
if (max(QTL.scan$lod) < SuggThresh){
  knit_exit("No QTL detected")
}
```
...
```{r}
TopAll<-names(IntegratedScores[order(IntegratedScores,decreasing = T)])
DFTopAll<-ScoresMatrixWeighted[TopAll,]
DFTopAll<-as.data.frame(DFTopAll)
DFTopAll$IntegratedScore<-as.vector(IntegratedScores[TopAll])
DFTopAll$FDR<-fdr[TopAll]
DFTopAll<-round(DFTopAll,3)
DFTopAll<-cbind(TopAll,DFTopAll)
DFTopAll
```
...
write.table(DFTopAll,file=OutputScore,sep="\t",col.names=T,row.names=T,quote=F)
...
```{r,fig.height=8,fig.width=8}
 for (i in 1:nrow(QTLsPeaks)){
   ...
   ylim2<-5
   chr<-as.character(QTLsPeaks[i,"Chr"])
   if (! chr %in% names(QTLMB[["geno"]]) && chr == "20"){chr<-"X"}
  ...
  OutputScore<-paste(gsub(".Rmd",'',opt$input),opt$id,opt$condition,opt$tissu,opt$phenotypetype,opt$weighting,opt$discretization,sep=".")
   ...
   maintext<-paste("Gene Scoring on chromosome:",chr)
   
ScorePlot(p1=Start-1,p2=End+1,PermResults=fdr,chro=chro,scan=QTL.scan,Main=maintext,ylim1=ylim1,ylim2=ylim2,SignifThresh,scorescale=1,Chromosome=Chromosome,Nomarker=T)
...
```{r,code=readLines(paste(SCRIPTDIR,"ScorePlot2.R",sep=''))}
```
