...
cat(c("Phenotype","QTL_Chromosome","Location_in_cM","Location_bp","Suggestive_Significant_QTL","PhenotypeNumber","Lodscore","Percentile","MarkerStart","MarkerEnd"),file="results/QTL_detection/phQTL/QTLlist.txt",sep='\t',append=TRUE)
cat('\n',file="results/QTL_detection/phQTL/QTLlist.txt",sep='',append=TRUE)
cat('',file="results/QTL_detection/phQTL/ProbaMatrix.txt")
cat(c("Pheno",''),file="results/QTL_detection/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
cat(rownames(scanMB),file="results/QTL_detection/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
cat('\n',file="results/QTL_detection/phQTL/ProbaMatrix.txt",append=TRUE)
# writing Lod Score Matrix
cat('',file='results/QTL_detection/phQTL/LODScoreMatrix.txt')
cat(c("Pheno",''),file="results/QTL_detection/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
cat(rownames(scanMB),file="results/QTL_detection/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
cat('\n',file="results/QTL_detection/phQTL/LODScoreMatrix.txt",append=TRUE)


#find phenotypes of interest
##grep("BXDvsRECdif.quant.out.n3d", names(QTL$pheno))
##names(QTL$pheno)[c(76,77,172,239,74,75,171)]
##names(QTL$pheno)[c(94,97,151,239)]
# run permutations
for (i in c(94,97,151,239)){
##for (i in 1:(length(QTL$pheno)-2)){
  ...
  ##load(paste("results/QTL_detection/phQTL/", name,".ScanScanPerm.Rdata",sep=''))
  ...
  pdf(paste("results/QTL_detection/phQTL/",name,".pdf",sep=''),width=8, height=5)
  ...
  cat(c(name,''), file="results/QTL_detection/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
  cat(probavector, file="results/QTL_detection/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
  cat('\n',file="results/QTL_detection/phQTL/ProbaMatrix.txt",append=TRUE)

  cat(c(name,''),file="results/QTL_detection/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
  cat(as.vector(QTL.scan$lod),file="results/QTL_detection/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
  cat('\n',file="results/QTL_detection/phQTL/LODScoreMatrix.txt",append=TRUE)

  save(QTL.scanperm,QTL.scan,file=paste("results/QTL_detection/phQTL/",name,".ScanScanPerm.Rdata",sep=''))
  if (length(peak)>0){
    for(j in 1:nrow(peak)){
      cat(c(name,peak[j,5],paste(peak[j,2],'-',peak[j,4],sep=''),paste('chr',peak[j,5],':',as.numeric(peak[j,8])*1000000,'-',as.numeric(peak[j,9])*1000000,sep=''),peak[j,7],i,peak[j,6],pctil(peak[j,6]),peak[j,1],peak[j,3]),file="results/QTL_detection/phQTL/QTLlist.txt",sep='\t',append=TRUE)
      cat('\n',file="results/QTL_detection/phQTL/QTLlist.txt",sep='',append=TRUE)
  	 }
  }
  cat('\n',file="results/QTL_detection/phQTL/QTLlist.txt",sep='',append=TRUE)
