...
OutputName<-"Liver.SD" 
ExpressionFile<-"/scratch/beegfs/monthly/SleepRegulation/BXD/BXD_mm10/exome/edgeR_normalized_log2CPM_Liver_SD.tab"
library(qvalue)
...
The genotype is taken from GeneNetwork and variant calling on our BXD expression data from liver and cortex.

```{r}
Geno<-read.table("/scratch/beegfs/monthly/SleepRegulation/BXD/BXD_mm10/genome/GNmm10.newgenotypes_subset_sorted.geno",header=TRUE)
```

## Expression

The molecular phenotypes are our normalized and log transformed raw BXD expression from liver or cortex.
...
Same with Mettl25 for LNSD and LSD.
The reason of this error is not completely understood.

```{r}
PhenoExclude<-"/scratch/beegfs/monthly/SleepRegulation/BXD/BXD_mm10/cis-eQTL/Cortex_sd/TranscripteQTLExclude.txt"
...
```{r}
Mice<-colnames(Geno)[-which(colnames(Geno) %in% c("Chr","Locus","cM","Mb","BXD005","BXD029","BXD032","BXD063"))]
Mice
```
"processed" means here that are removed:

* chromosome number ("Chr")
* gene position, and length ("Locus","cM","Mb")
* expression data for 4 old BXD lines ("BXD005","BXD029","BXD032","BXD063") and 2 parental replicates ("B61","DB1")
...
```{r}
system(paste(paste(SCRIPTDIR,"Create_BED_UCSC_mm10.py",sep=''),ExpressionData,GenotypeData,paste(ExpressionData,'.bed',sep='')))
...
```{r}
# launch this script that performs the eQTL analysis using fastQTL.
...
##  system(paste("echo 'Stab2 19 0.711218 1.41551 41.9348 rs29341107 842923 0.00261049 6.13439 0.000999001 0.00547887' >>",paste("ciseQTL",OutputName,"txt",sep=".")))
##} else if (OutputName == "Liver.SD"){
##  system(paste("echo 'Stab2 15 0.898049 2.24287 36.1828 rs30246660 93718 4.02702e-05 -42.0118 0.001998 6.18041e-05' >>",paste("ciseQTL",OutputName,"txt",sep=".")))
##}
...
Here we use the qvalue package (correction for testing many phenotypes) as suggested by FastQTL authors
