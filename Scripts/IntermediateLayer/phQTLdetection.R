#GOAL This R script detects phQTL using R/qtl packages.

#INPUT The input file is the formatted genotypes where position is in cM (for example "results/genome/formatted_genotypes.csv") and in MB (for example "results/genome/formatted_genotypes.csv.MBDistance.QTLR") and formatted phenotypes.
...
##QTL <- read.cross("csv",file="results/genome/formatted_genotypes.csv",genotypes=c("BB", "BD", "DD","EE","FF"),allele=c("B", "D"),estimate.map=FALSE,na.strings=c("NA","-"))
QTL <- read.cross("csv",file="BXD_mm10/phQTL/formatted_genotypesmm10ANDphenotypes.csv",genotypes=c("BB", "BD", "DD","EE","FF"),allele=c("B", "D"),estimate.map=FALSE,na.strings=c("NA","-"))
#33 individuals, 11186 markers, 350 phenotypes, Cross type: f2, omitting heterozygotes in X chromosome.
...
##QTLMB <- read.cross("csv",file="results/genome/formatted_genotypes.csv.MBDistance.QTLR",genotypes=c("BB", "BD", "DD","EE","FF"),allele=c("B", "D"),estimate.map=FALSE,na.strings=c("NA","-"))
QTLMB <- read.cross("csv",file="BXD_mm10/phQTL/formatted_genotypesmm10ANDphenotypes.csv.MBDistance.QTLR",genotypes=c("BB", "BD", "DD","EE","FF"),allele=c("B", "D"),estimate.map=FALSE,na.strings=c("NA","-"))
...
cat('',file="results/QTL_detection/phQTL/QTLlist.txt")
cat(c("Phenotype","QTL_Chromosome","Location_in_cM","Location_bp","Suggestive_Significant_QTL","PhenotypeNumber","Lodscore","Percentile","MarkerStart","MarkerEnd"),file="BXD_mm10/phQTL/QTLlist.txt",sep='\t',append=TRUE)
cat('\n',file="BXD_mm10/phQTL/QTLlist.txt",sep='',append=TRUE)
cat('',file="BXD_mm10/phQTL/ProbaMatrix.txt")
cat(c("Pheno",''),file="BXD_mm10/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
cat(rownames(scanMB),file="BXD_mm10/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
cat('\n',file="BXD_mm10/phQTL/ProbaMatrix.txt",append=TRUE)
# writing Lod Score Matrix
cat('',file='BXD_mm10/phQTL/LODScoreMatrix.txt')
cat(c("Pheno",''),file="BXD_mm10/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
cat(rownames(scanMB),file="BXD_mm10/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
cat('\n',file="BXD_mm10/phQTL/LODScoreMatrix.txt",append=TRUE)


#find phenotypes of interest
##grep("BXDvsRECdif.quant.out.n3d", names(QTL$pheno))
##names(QTL$pheno)[c(76,77,172,239,74,75,171)]
##names(QTL$pheno)[c(95,97,151,239)]
# run permutations
for (i in c(95,97,151,239)){
##for (i in 1:(length(QTL$pheno)-2)){
  ...
  ##load(paste("BXD_mm10/phQTL/", name,".ScanScanPerm.Rdata",sep=''))
  ...
  pdf(paste("BXD_mm10/phQTL/",name,".pdf",sep=''),width=8, height=5)
  ...
  cat(c(name,''), file="BXD_mm10/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
  cat(probavector, file="BXD_mm10/phQTL/ProbaMatrix.txt",sep='\t',append=TRUE)
  cat('\n',file="BXD_mm10/phQTL/ProbaMatrix.txt",append=TRUE)

  cat(c(name,''),file="BXD_mm10/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
  cat(as.vector(QTL.scan$lod),file="BXD_mm10/phQTL/LODScoreMatrix.txt",sep='\t',append=TRUE)
  cat('\n',file="BXD_mm10/phQTL/LODScoreMatrix.txt",append=TRUE)

  save(QTL.scanperm,QTL.scan,file=paste("BXD_mm10/phQTL/",name,".ScanScanPerm.Rdata",sep=''))
  if (length(peak)>0){
    for(j in 1:nrow(peak)){
      cat(c(name,peak[j,5],paste(peak[j,2],'-',peak[j,4],sep=''),paste('chr',peak[j,5],':',as.numeric(peak[j,8])*1000000,'-',as.numeric(peak[j,9])*1000000,sep=''),peak[j,7],i,peak[j,6],pctil(peak[j,6]),peak[j,1],peak[j,3]),file="BXD_mm10/phQTL/QTLlist.txt",sep='\t',append=TRUE)
      cat('\n',file="BXD_mm10/phQTL/QTLlist.txt",sep='',append=TRUE)
  	 }
  }
  cat('\n',file="BXD_mm10/phQTL/QTLlist.txt",sep='',append=TRUE)
